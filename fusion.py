...
class Fusion(object):
    ...
    def make_intrinsic_mat(self,param):
        ...
        for i in range(CAMMERA_NUM):
            intrinsic=np.array([[param[i][0],0,param[i][2],0],[0, param[i][1], param[i][3],0],[0,0,1,0],[0,0,0,1]])
    ...
    def calibration(self):
        ...
        for i in range(len(annos2d)):
            ...
            for cam_num, img in enumerate(annos2d[i]["imgs"]):
                ...
                img_to_3d=np.linalg.inv(self.current_extrinsics[cam_num])*np.linalg.inv(self.current_intrinsics[cam_num])*img
