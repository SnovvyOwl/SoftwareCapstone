...
if __name__=="__main__":
    ...
    ### DATALOAD
    with open("data/waymo/waymo_processed_data/segment-1024360143612057520_3580_000_3600_000_with_camera_labels/img/camera_segment-1024360143612057520_3580_000_3600_000_with_camera_labels.pkl","rb") as f:
    ...
    for i,label in enumerate(gt["labels"]):
        box=gt["bboxes"][i]
        if label=="Vehicle":
            cv2.rectangle(t_img,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),(255-gt_padd,gt_padd,gt_padd))
        if label=="Pedestrian":
            cv2.rectangle(t_img,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),(gt_padd,255,gt_padd))
        if label=="Sign":
            cv2.rectangle(t_img,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),(gt_padd,gt_padd,255-gt_padd))
        if label=="Cyclist":
            cv2.rectangle(t_img,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),(gt_padd,255-gt_padd,255-gt_padd))
        if label=="Unknown":
            cv2.rectangle(t_img,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),(255-gt_padd,gt_padd,255-gt_padd))
    # PREDICTION
