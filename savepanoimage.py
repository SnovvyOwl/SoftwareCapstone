...
def save_images(frame,sequence_name,cur_save_dir,cnt):
    ...
    for camera_num in camera_name: #프레임당이미지 다섯개
        ...
        p.append(Process(target=save_image,args=(frame[camera_num],path)))
    for i in range(5):
        p[i].start()
    for i in range(5):
        p[i].join()
...
def save_image(imagefile,path):
    camera_img=Image.fromarray(imagefile)
...
if __name__=="__main__":
    datapath=Path("/home/seongwon/SoftwareCapstone/PV_R-CNN/data/waymo/raw_data/segment-1024360143612057520_3580_000_3600_000_with_camera_labels.tfrecord")
    savepath=Path("/home/seongwon/SoftwareCapstone/PV_R-CNN/data/waymo/waymo_processed_data")
    sampled_interval=1
    has_label=True
